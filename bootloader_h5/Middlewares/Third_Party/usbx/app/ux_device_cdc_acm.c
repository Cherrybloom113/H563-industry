...
static SemaphoreHandle_t g_USBXSendSemaphore;
static QueueHandle_t g_USBXReciveQueue;
...
static UINT ux_device_class_cdc_acm_write_callback(struct UX_SLAVE_CLASS_CDC_ACM_STRUCT *cdc_acm, UINT status, ULONG length)
{
	...
	xSemaphoreGive(g_USBXSendSemaphore);
...
static UINT ux_device_class_cdc_acm_read_callback(struct UX_SLAVE_CLASS_CDC_ACM_STRUCT *cdc_acm, UINT status, UCHAR *data_pointer, ULONG length)
{
	...
	{
		xQueueSend(g_USBXReciveQueue, (const void *)&data_pointer[i], 0);
	}
...
VOID USBD_CDC_ACM_Activate(VOID *cdc_acm_instance)
{
  ...
  ux_device_class_cdc_acm_ioctl(cdc_acm, UX_SLAVE_CLASS_CDC_ACM_IOCTL_TRANSMISSION_START, (VOID *)&parameter);
	...
	if(!g_USBXSendSemaphore)
	{
			g_USBXSendSemaphore = xSemaphoreCreateBinary();
			g_USBXReciveQueue = xQueueCreate(10240, 1);
	}
...
int ux_device_cdc_acm_send(uint8_t *datas, uint32_t len, uint32_t timeout)
{
...

    {
        ...
        {
            if(pdTRUE == xSemaphoreTake( g_USBXSendSemaphore, timeout))
							return 0;
						else
							return -1;
        }
...
{
	if(g_USBXReciveQueue)
		{
			if(pdPASS == xQueueReceive(g_USBXReciveQueue, datas, timeout))
				return 0;
			else
				return -1;			
		}
...
int ux_device_cdc_acm_flush(void)
{
	...
	while(1)
	{
		if(pdTRUE != xQueueReceive(g_USBXReciveQueue, data, 0))
		{
			count++;
			break;
		}
